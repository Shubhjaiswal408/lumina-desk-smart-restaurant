...
| **Muted** — still takes orders, just stays silent | **Done** — settled, with a feedback QR |
...
- **Muted** — still takes orders and updates the screen, but says nothing. The
  panel shows a 🔇 *Voice muted* badge so a guest isn't left talking to a screen
  that has quietly stopped answering.
- **Off** — ignores the wake word entirely; staff can still run the table from
  the console.
